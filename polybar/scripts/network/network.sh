...
function initialization() {
	source "$DIR/network.conf"
	{ [[ -f "$DIR/network.rasi" ]] && RASI_DIR="$DIR/network.rasi"; } || { [[ -f "${XDG_CONFIG_HOME:-$HOME/.config}/rofi/rofi-network-manager.rasi" ]] && RASI_DIR="${XDG_CONFIG_HOME:-$HOME/.config}/rofi/rofi-network-manager.rasi"; } || exit
...
function rofi_cmd() {
	rofi -dmenu -i -location "$LOCATION" -yoffset "$Y_AXIS" -xoffset "$X_AXIS" $3 -theme "$RASI_DIR" -theme-str 'textbox-prompt-colon{str:"'"$PROMPT"':";}'"$4"''
}
